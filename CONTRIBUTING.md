...
- 分支保持短生命周期：一次 PR 只解决一个明确问题，及时合并，不长期占位或累积无关改动。
- 改动前先搜索 Issue 和现有 PR 声明认领，避免多人同时改同一模块；涉及同一模块或契约的并行改动，先开 Issue 对齐方案与合并顺序。
- PR 之间存在依赖时，在描述中标注依赖的 PR/Issue 编号，被依赖方先合并；不得把未合并依赖的变更悄悄包含进另一个 PR。
- 合并前必须将分支同步到最新 `main`（Update branch 或本地 rebase）并重新触发 CI；分支最新且 CI 全部通过后才允许合并。
- 解决冲突时以最新 `main` 为基准更新分支并重跑相关检查；冲突涉及他人改动时与相关作者确认语义。
- 发布不打冻结窗口：`v*` 标签随时可从 `main` 打出，发布与日常合并不互斥。
- 合并后远端分支自动删除，本地分支应同步清理。
