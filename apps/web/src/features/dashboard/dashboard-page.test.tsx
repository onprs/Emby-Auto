...
function systemMetrics() {
  return {
    ...
    disks: [{ path: 'D:', usedBytes: 600 * gibibyte, totalBytes: 1000 * gibibyte, usedPercent: 60 }],
    samples: [
...
describe('DashboardPage attention', () => {
  it('shows actionable acquisition information, host resource charts and failed operation history', async () => {
    ...
    expect(within(resources).getByLabelText('磁盘资源图表')).toHaveTextContent('D:');
    expect(within(resources).getByLabelText('磁盘资源图表')).toHaveTextContent('60%');
