...
func TestRSSCascadeStopsUnfinishedAndPreservesImportedIntegration(t *testing.T) {
	...
	if _, err := pool.Exec(ctx, `INSERT INTO episode_mapping_profiles (id, series_id, name, version, source_season_lengths, active, created_by, decision_source) VALUES ($1, $2, 'p', 1, ARRAY[13], true, $3, 'user')`, mappingProfileID, seriesID, actorID); err != nil {
		t.Fatal(err)
	}
