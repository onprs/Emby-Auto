...
VALUES ($1, $3, 1, 'Episode 1'), ($2, $3, 2, 'Episode 2')`, episodeIDs[0], episodeIDs[1], seasonID); err != nil {
	...
	if _, err := pool.Exec(ctx, `
INSERT INTO episode_mapping_profiles (id, series_id, name, version, source_season_lengths, active, created_by)
VALUES ($1, $3, 'old', 1, ARRAY[2], true, $4),
       ($2, $3, 'new', 1, ARRAY[2], true, $4)`, oldProfileID, newProfileID, seriesID, actorID); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `
...
VALUES ($1, $3, 'Requested Show'), ($2, $4, 'Other Show')`, seriesID, otherSeriesID, tmdbSeriesID, tmdbSeriesID+1); err != nil {
	...
	if _, err := pool.Exec(ctx, `
INSERT INTO episode_mapping_profiles (id, series_id, name, version, source_season_lengths, active, created_by)
VALUES ($1, $2, 'Incomplete S01', 1, ARRAY[2], true, $3)`, incompleteProfileID, seriesID, actorID); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `
INSERT INTO episode_mappings (id, profile_id, source_season, source_episode, absolute_episode, target_episode_id, mapping_status, match_source)
...
func seedRSSMappingProfile(
	...
	if _, err := pool.Exec(ctx, `
INSERT INTO episode_mapping_profiles (id, series_id, name, version, source_season_lengths, active, created_by)
VALUES ($1, $2, $3, 1, ARRAY[2], true, $4)`, profileID, seriesID, name, actorID); err != nil {
		t.Fatal(err)
	}
	if _, err := pool.Exec(ctx, `
INSERT INTO episode_mappings (id, profile_id, source_season, source_episode, absolute_episode, target_episode_id, mapping_status, match_source)
